...
class BaseFaqTestCase(TestCase):
    def setUp(self):
        ...
        self.url_redirect_1 = reverse(
            'department_faq', kwargs={'department': self.department_1.slug}
        )
        self.department_2 = Department.objects.create(
            name='Technology Support', slug=slugify('Technology Support')
        )
        self.url_redirect_2 = reverse(
            'department_faq', kwargs={'department': self.department_2.slug}
        )
        self.department_3 = Department.objects.create(
            name='Mitigating Circumstances',
            slug=slugify('Mitigating Circumstances'),
        )
        self.url_redirect_3 = reverse(
            'department_faq', kwargs={'department': self.department_3.slug}
        )
    ...
    def test_base_faq_uses_correct_context(self):
        ...
        self.assertIn(
            ...
            [
                department_1.name
                for department_1 in response.context['object_list']
            ],
        )
        self.assertIn(
            self.department_1.slug,
            [
                department_1.slug
                for department_1 in response.context['object_list']
            ],
        ...
        self.assertIn(
            ...
            [
                department_2.name
                for department_2 in response.context['object_list']
            ],
        )
        self.assertIn(
            self.department_2.slug,
            [
                department_2.slug
                for department_2 in response.context['object_list']
            ],
        ...
        self.assertIn(
            self.department_3.name,
            [
                department_3.name
                for department_3 in response.context['object_list']
            ],
        )
        self.assertIn(
            self.department_3.slug,
            [
                department_3.slug
                for department_3 in response.context['object_list']
            ],
